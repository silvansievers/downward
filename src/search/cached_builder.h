...
protected:
    virtual std::shared_ptr<T> build(const std::shared_ptr<AbstractTask> &task) const = 0;
public:
    CachedBuilder()
    ...
    std::shared_ptr<T> get_built_element(const std::shared_ptr<AbstractTask> &task) const {
        TaskProxy task_proxy(*task);
        if (!constructed_elements[task_proxy]) {
            constructed_elements[task_proxy] = build(task);
        }
