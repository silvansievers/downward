...
TieBreakingOpenListFactory::TieBreakingOpenListFactory(const Options &opts)
    : OpenListFactory(opts.get<bool>("pref_only")),
      evaluator_builders(opts.get_list<shared_ptr<PluginBuilder<Evaluator>>>("evals")),
      allow_unsafe_pruning(opts.get<bool>("unsafe_pruning")) {
}

unique_ptr<StateOpenList>
TieBreakingOpenListFactory::create_state_open_list(PluginVariableAssignment &variable_context, const shared_ptr<AbstractTask> &task) {
    vector<shared_ptr<Evaluator>> evaluators;
    evaluators.reserve(evaluator_builders.size());
    for (auto &builder : evaluator_builders) {
        evaluators.push_back(builder->build(variable_context, task));
    }
...
static shared_ptr<OpenListFactory> _parse(OptionParser &parser) {
    ...
    parser.add_list_option<shared_ptr<PluginBuilder<Evaluator>>>("evals", "evaluators");
