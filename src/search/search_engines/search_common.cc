...
create_astar_open_list_factory_and_f_eval(const Options &opts) {
    // TODO: use correct task
    shared_ptr<AbstractTask> task = nullptr;
    shared_ptr<GEval> g = make_shared<GEval>(task);
    shared_ptr<Evaluator> h = opts.get<shared_ptr<EvaluatorBuilder>>("eval")->build(task);
    shared_ptr<Evaluator> f = make_shared<SumEval>(task, vector<shared_ptr<Evaluator>>({g, h}));
