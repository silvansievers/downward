...
EagerSearchBuilder::EagerSearchBuilder(const options::Options &opts)
    : SearchEngineBuilder(
      ...
      f_evaluator_builder(opts.get<shared_ptr<PluginBuilder<Evaluator>>>("f_eval", nullptr)),
      preferred_operator_evaluator_builders(opts.get_list<shared_ptr<PluginBuilder<Evaluator>>>("preferred")),
...
shared_ptr<SearchEngine> EagerSearchBuilder::build(
    ...
    for (auto &builder : preferred_operator_evaluator_builders) {
        preferred_operator_evaluators.push_back(builder->build(variable_context, task));
    }
    return make_shared<EagerSearch>(
        open_list_builder->create_state_open_list(variable_context, task),
        f_evaluator_builder->build(variable_context, task),
        preferred_operator_evaluators,
        lazy_evaluator_builder ?
            lazy_evaluator_builder->build(variable_context, task) : nullptr,
